...
class TestEnhancedSearch:
    def test_finds_observation_with_natural_language(self, tmp_path: object) -> None:
        ...
        conn.execute("CREATE TABLE observations (id TEXT PRIMARY KEY, timestamp INTEGER, content TEXT, title TEXT, metadata TEXT, idempotency_key TEXT, topic_key TEXT, project TEXT, type TEXT, revision_count INTEGER DEFAULT 1, session_id TEXT)")
        conn.execute("CREATE VIRTUAL TABLE observations_fts USING fts5(content, title, topic_key, content=observations, tokenize='unicode61')")
        conn.execute("CREATE TRIGGER observations_ai AFTER INSERT ON observations BEGIN INSERT INTO observations_fts(rowid, content, title, topic_key) VALUES (new.rowid, new.content, new.title, new.topic_key); END")
        conn.commit()

        import time
        test_data = [
            ("aaaa0001", "Fork sessions are created in /tmp/fork-live-sessions/ directory"),
            ("aaaa0002", "Archive old memory observations after pilot is complete or keep them live for reference"),
            ("aaaa0003", "Observations should be updated when the topic evolves, replaced when the content is obsolete"),
            ("aaaa0004", "Status field vs stability metric: stability wins when retrieving cached observations"),
            ("aaaa0005", "Fallback contracts for tools ensure no silent failures in the orchestration pipeline"),
        ]
        for short_id, content in test_data:
            full_id = f"{short_id}-0000-0000-0000-000000000000"
            conn.execute("INSERT INTO observations (id, timestamp, content) VALUES (?, ?, ?)", (full_id, int(time.time() * 1000), content))
        conn.commit()
        ...
        for query, expected_prefix in test_cases:
            ...
            assert expected_prefix in ids, f"Query '{query}' should find {expected_prefix}, got {ids}"
