...
class TestAllEndpoints:
    ...
    def test_agents_create(self, client, auth_headers):
        client.post('/api/v1/agents/sessions', headers=auth_headers, json={"agent_type": "opencode", "task": "test"})
...
class TestCoveragePush:
    def test_memory_detailed(self, client, auth_headers):
        client.post('/api/v1/memory', headers=auth_headers, json={"type": "observation", "content": "test data", "metadata": {"key": "value"}})
    ...
    def test_workflow_detailed(self, client, auth_headers):
        client.post('/api/v1/workflow/outline', headers=auth_headers, json={"task": "test", "context": {}})
        client.get('/api/v1/workflow/status', headers=auth_headers)
    ...
    def test_processes_variations(self, client, auth_headers):
        ...
        client.post('/api/v1/processes', headers=auth_headers, json={"name": "test", "script": "app.js", "instances": 1})

    def test_integrations_variations(self, client, auth_headers):
        client.post('/api/v1/integrations/branch-review/command', headers=auth_headers, json={"command": "init"})
        client.post('/api/v1/integrations/branch-review/command', headers=auth_headers, json={"command": "explore", "args": {}})

    def test_webhooks_variations(self, client, auth_headers):
        client.post('/api/v1/webhooks', headers=auth_headers, json={"url": "https://test.com", "events": ["push"]})
...
class TestFinalPush:
    def test_memory_all_flows(self, client, auth_headers):
        client.post('/api/v1/memory', headers=auth_headers, json={"type": "test", "content": "data1"})
        client.get('/api/v1/memory', headers=auth_headers)
    ...
    def test_agents_all_flows(self, client, auth_headers):
        ...
        client.post('/api/v1/agents/sessions', headers=auth_headers, json={"agent_type": "opencode", "task": "test1"})
    ...
    def test_processes_all_flows(self, client, auth_headers):
        client.get('/api/v1/processes', headers=auth_headers)
        client.post('/api/v1/processes', headers=auth_headers, json={"name": "p1", "script": "s.js"})
    ...
    def test_webhooks_all_flows(self, client, auth_headers):
        ...
        client.post('/api/v1/webhooks', headers=auth_headers, json={"url": "http://a.com", "events": ["e1"]})
...
class TestMemoryQuery:
    ...
    def test_memory_query_with_all_params(self, client, auth_headers):
        client.get('/api/v1/memory/query?agent=a&run=r&event-type=e&limit=5&scan-limit=100&since=24h', headers=auth_headers)
