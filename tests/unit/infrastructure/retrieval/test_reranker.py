...
def _observation(
    obs_id: str, obs_type: str, origin_hint: str = "", status: str = "", stability: str = ""
) -> Observation:
    return Observation(
        id=obs_id,
        timestamp=1,
...
def test_rerank_by_intention_boosts_historical_memory_discovery() -> None:
    ...
    reranked = rerank_by_intention(
        results, "historical", "why paths get hyphens instead of underscores"
    )
    assert reranked[0].id == "2"
...
def test_rerank_by_intention_boosts_operational_procedure() -> None:
    ...
    reranked = rerank_by_intention(
        results, "operational", "how do I kick off branch review from cli"
    )
    assert reranked[0].id == "2"
