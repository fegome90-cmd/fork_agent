...
class TestMessageStoreCleanupExpired:
    ...
    def test_cleanup_removes_expired_messages(self, tmp_path: Path) -> None:
        ...
        db_conn = sqlite3.connect(str(db_path))
        db_conn.execute("UPDATE messages SET expires_at = 0 WHERE id = ?", (msg.id,))
        db_conn.commit()
        db_conn.close()
...
class TestMessageStoreGetDbPath:
    ...
    def test_get_db_path_returns_path(self, tmp_path: Path) -> None:
        ...
        result = store._connection._config.db_path
    ...
    def test_default_db_path(self) -> None:
        ...
        # Should end with memory.db (standard fork DB name)
        assert str(result).endswith("memory.db")
