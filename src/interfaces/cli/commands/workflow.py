...
@app.command("execute", help="Execute workflow plan tasks. Use --messaging to send PROGRESS messages to other agents for coordination.")
def execute(
...
@app.command("status", help="Show current workflow state across all phases (plan, execute, verify).")
def status() -> None:
    plan_path = get_plan_state_path()
    exec_path = get_execute_state_path()
    verify_path = get_verify_state_path()
